/*
 * tcom-engine.js
 * last mod: 06-02-2016#10:20
 * NO slider su Chrome vers. 44(solo Android)
 * setChangeTicketUrlMobile
 * slider pager mobile
 * slider x mobile con filtro su slide visibili
 * fix crossbar-menu-height to auto
 * fixOldIeCssIssues ie8
 * fix fixBoxColHeight Chrome vers. >=52
...
var tcom14 = {
    ...
    init : function(){
		...
		//tcom14.fixOldIeCssIssues(); // ie8
		
		// Chrome >=52
		try{ setTimeout(function(){ tcom14.fixBoxColHeight('div.threecol'); tcom14.fixBoxColHeight('div.twocol'); }, 1500); }catch(e){}
		
		// 'loadCheckbox' fix formcruscotto etc.
		setTimeout(function(){ if(typeof loadCheckbox!="undefined"){ loadCheckbox(); } }, 600);
    },
            ...
            $(e).bind('click', function(ev){
                ...
                if(myElem.hasClass('open')){
                    ...
                    if(!Modernizr.csstransitions){
                        ...
                        $(myElem).animate({'height':'auto'},500,'cool',function(){
                            $(this).addClass('open');
                        });
                    }else{
    ...
    emotional : {
        ...
        init : function(){
			...
			//console.log("=> nItem=["+tcom14.emotional.nItem+"] ");
            $('ul.slider').hide();
            /* setto larghezza UL */
            ...
            if($(window).width() <= tcom14.mSize){
				...
				$(".mobPager > .prev").bind("click", function(){ tcom14.emotional.prev(); });
				$(".mobPager > .next").bind("click", function(){ tcom14.emotional.next(); });
			}
        },
...
*/
    ...
    checkForm : function(e){
		...
		//Controllo se la data di partenza ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¨ inserita e se il formato ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¨ corretto
		var delim1 = '-';
		var delim2 = '-';
	...
	if($('#biglietti_data_p').length > 0){
		...
		var dataRitornoES = giornoRitorno +"/"+ meseRitorno +"/"+  annoRitorno;
		 //Controllo che la data sia corretta cioÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¨ il mese < 12 e il giono giusto
        ...
        /* seleziono dove cercare le cittÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢  */
    ...
    replaceText : function(e){
        ...
        if($.inArray(n,_cfg.notReplaceSAN)== -1){
			...
			/*Controllo che dopo s. non ci si a giÃƒÆ’Ã†â€™  lo spazio*/
			if(n.split("",3)!= 'S,., '){
   ...
   addPlaceHolder : function(){
    ...
    // fix H-SpallaDX: imposto altezza MINIMA del wrapBody se di lunghezza minore della spallaDX viene coperto
	...
	setChangeTicketUrlMobile : function(){
		if($(window).width()<=tcom14.mSize){
				if($('.ricercaAvanzata').attr('href').indexOf('lang=it')>0){
					$("a#MODBIG").attr('href','https://www.lefrecce.it/msite/#modificaBiglietto');
					/*Modifica per inserire i bottoni su ricerca avanzata*/
					$('.ricercaAvanzata label').remove();
					$('.ricercaAvanzata img').remove();
					$('.ricercaAvanzata').append("<label>RICERCA DESKTOP&nbsp;</label>");
					$('.ricercaAvanzata').append("<img src='/cms-file/immagini/trenitalia2014/Homepage/ico_ric_avanz.png' alt='Ricerca Desktop'>");	
				}				
				else if($('.ricercaAv').attr('href').indexOf('lang=en')>0){
					$("a#MODBIG").attr('href','https://www.lefrecce.it/msite/?lang=en#modificaBiglietto');
					$('.ricercaAv img').remove();
					$('.ricercaAv').append("<img src='/cms-file/immagini/trenitalia_2014_eng/Homepage/ric-desktop.png' alt='Advanced search'>");
				}
				else if($('.ricercaAv').attr('href').indexOf('lang=de')>0){
					//$("a#MODBIG-EN").attr('href','https://www.lefrecce.it/msite/?lang=de#modificaBiglietto');
					$('.ricercaAv img').remove();
					$('.ricercaAv').append("<img src='/cms-file/immagini/trenitalia2014_de/homepage/ric-desktop.png' alt='Desktop suchen'>");
				}
				else if($('.ricercaAv').attr('href').indexOf('lang=fr')>0){
					//$("a#MODBIG-EN").attr('href','https://www.lefrecce.it/msite/?lang=fr#modificaBiglietto');
					$('.ricercaAv img').remove();
					$('.ricercaAv').append("<img src='/cms-file/immagini/trenitalia2014_fr/homepage/ric-desktop.png' alt='Desktop recherche'>");
				}
		}
    },
	...
	fixOldIeCssIssues : function(){
		try{
			if($("html").hasClass("ie8")){
				var ie8CssFixes=[
					'<style> #AbbCarnetMobile, .tabLinkMobile, .header_mobile{display:none;} </style>',
					'<style> .boxBanner.no-mobile{ margin-top:75px;} </style>',
					'<style> .labelABBCAR3{padding-top:50px !important} </style>',
					'<style> .tabLinkCL{float:left;width:155px !important;height:40px !important;border:1px solid #dfdfdf !important} </style>',
					'<style> .emotional .cruscotto FIELDSET.biglietti_from_to INPUT{width:170px !important} </style>',
					'<style> .icoScambioDest{position:absolute;left:240px;top:110px} </style>',
					'<style> .emotional .cruscotto FIELDSET.biglietti_data_ora .contdata{height:33px !important} </style>',
					'<style> .emotional .cruscotto FIELDSET.biglietti_data_ora .contdata INPUT{padding:5px !important} </style>',
					'<style> .styled-select SELECT{height:28px !important}</style>',			
					'<style> .emotional .cruscotto .ricercaAv{} </style>',
					'<style> .boxBanner{margin-top:0px !important}</style>',
					'<style> .tabLinkCL{width:158.5px !important}</style>',
					'<style> .emotional .cruscotto .ricercaAv{font-size:1em !important;line-height:1.9em !important;margin-bottom:10px !important}</style>',
					'<style> .linkCRUS A{top:5px !important}</style>',
					'<style> .emotional .cruscotto FIELDSET.biglietti_ar LABEL{position:relative;top:9px}</style>'];
				$.each(ie8CssFixes, function(i, v){ $('body').append(v); });
			}
		}catch(e){}finally{}
	},
			...
			if(isNwChrome){
				$(boxColSelector).each(function(){
					...
					$(this).find('div.box').each(function(i, curVal1){
						...
						if(curBoxH>curBoxMaxH){ curBoxMaxH=curBoxH; }
					});
					$(this).find('div.box').each(function(j, curVal2){
						if(j!=1){ $(curVal2).css('display', 'inline-block'); 
						}else{ $(curVal2).css('display', 'block'); }						

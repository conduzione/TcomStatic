...
var ferrovie = {
	...
	init : function() {
        ...
        $('.mainEmoSlider ul.slider, .soste-visual ul.slider').bxSlider({
            controls:false,
            pager:true
		});
        ...
        $('.mediaDetailSlider ul.slider').bxSlider({
            minSlides: 1,
            maxSlides: 5,
            slideWidth: 135,
            slideMargin: 10,
        ...
        $('.mediaDetailSlider ul.slider > li > a').click(function(e){
            e.preventDefault();
            $(this).addClass('active').closest('li').siblings('li').find('> a').removeClass('active');
            $('.mediaDetailContent:first').html($('span',this).clone());
        }).eq(0).click();
		...
		this.checkDevice();
		$('.article-slider ul.slider').bxSlider({
            controls:false,
            pager:true,
            adaptiveHeight: false,
            onSliderLoad:function(){
                ferrovie.normalizeCarousels();
            }
        });
		...
		$('.leafContent img').not('.onlyImage').each(function() {
        ...
        $('.media-list.images').each(function(){
            var $t = $(this),
                $els = $t.find('> *'),
                $togglables;
            $togglables = $els.filter(':gt(6)');
            $togglables = $togglables.filter(':lt('+($togglables.size()-1)+')');
            $togglables.hide();
            $t.find('.vedi-tutti').click(function(e){
                e.preventDefault();
    ...
    normalizeCarousels:function(){
        //setTimeout(function(){
            $('.article-slider.border ul.slider').each(function(i,o){
                var _h=0;
                $(o).children('li').each(function(j,p){
                    var h=$(p).height();
                    if (h>_h) _h=h;
                }).height(_h);
            });
    ...
    setMobileMenu : function(){
        // setting span actions
        $('div.menu > ul > li > a > span').click(function(event){
            event.preventDefault();
        ...
        $('div.menu .menuTit > a > span').click(function(event){
            event.preventDefault();
            event.stopPropagation();
            var isAct=$(this).parents('div.menuTit').eq(0).hasClass('active');
            $(this).parents('ul').eq(0).find('div.menuTit,ul').removeClass('active');
            if(!isAct)
                $(this).parents('div.menuTit').eq(0).addClass('active').nextAll('ul').eq(0).addClass('active')
        });

        // removing unnecessary spans
        $('div.menu > ul > li').each(function(i,o){
            if($(o).children('ul').size()===0)
                $(o).children('a').children('span').remove();
            else
                $(o).find('div.menuTit').each(function(j,t){
                    if($(t).nextUntil('div.menuTit','ul').size()===0)
                        $(t).children('a').children('span').remove();
                });
        });

        // set menu height
        ferrovie.isMobile ?
            $('div.menu').height($('body > .content').height()) : $('div.menu').css("height","auto");
    },
	...
	setChiudiSubMenu : function() {
		if ($(".subMenuClose").length > 0) {
			$(".subMenuVoiceGrp,.subMenuSites .subMenuClose a").click(function() {
				ferrovie.subMenuGrp();
				return false;
			});
			$(".subMenuVoiceScl,.subMenuSocial .subMenuClose a").click(function() {
				ferrovie.subMenuScl();
				return false;
			});
		}
	...
	resizeMenu : function() {
		...
		*/
        var _menu = $('.menu > ul > li'),
            _menu_ul = $('.menu > ul > li > ul'),
            _menu_li = $('.menu > ul > li > ul > li');
        ...
        if (ferrovie.isMobile) {
            ...
            setTimeout(function(){
                var _h=0;
                $('.menu > ul > li.hover > ul > li').each(function(i,o){
                    var h=$(o).height();
                    if(h>_h) _h=h;
                });
                $('.menu > ul > li.hover > ul > li').height(_h);
            },10);
        }
	},
	...
	leafFormDatepickers : function(classContent) {
        ...
        $('div.clndr').click(function(event){
            event.preventDefault();
            $(this).find('input[type=text]').focus();
        });
        $('div.clndr input').datepicker({
            changeMonth : true,
            numberOfMonths : 1,
            dateFormat: "dd.mm.yy"
        });
    ...
    socialPresence:function(){
        // menu presence mobile
        $('.socialBox > h5,.socialBox .net > h6').click(function(event){
            event.preventDefault();
            $(this).parent().toggleClass('active');
        });

        // tooltips
        $('.socialBox .socials .net a').tooltipster({
            content:"",
            animation:"fade",
            delay:0,
            speed:150,
            ...
            functionReady: function(origin, content) {
                var cnt=$(origin).next().html();
                origin.tooltipster('content',cnt);
            },
            functionBefore:function(origin,continueTooltip) {
                if(ferrovie.isMobile) return false;
                else continueTooltip();
            }
        });
        ...
        if(!ferrovie.isMobile){
            $('.socialWall').masonry({
                ...
                itemSelector: '.socialBrick'
            });
        }

        //load more
        var loading=false;
        $('.socialWall-wrap a.loadMore').click(function(event){
            event.preventDefault();
            var A=this;
            if(!loading){
                $(A).prev('div.loading').show();
                $.ajax({
                    url : "./inc/social-bricks.html"
                }).done(function(data) {
                    $(A).prev('div.loading').hide();
                    $('.socialWall').append(data).masonry('reloadItems').masonry('layout');
                });
            }
        });
    ...
    indici:function(){
        if($('div.indice').size() > 0){
            if(ferrovie.isTablet || ferrovie.isMobile){
                var maxRight= $('div.indice').width() - $('div.indice .cnt').width();
                $('div.indice .cnt').draggable({
                    axis:"x",
                    drag:function(event,ui){
                        if(ui.position.left >= 0)
                            ui.position.left=0;
                        else if(ui.position.left<=maxRight)
